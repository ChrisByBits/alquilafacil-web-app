...
onMounted(async() => {
  ...
  // Load contract instance for this reservation
  try {
    contractInstance.value = await contractsService.getContractInstanceByReservationId(reservation.value.id);
  } catch (error) {
    console.log('No contract found for this reservation');
  }
});
...
const downloadContractPdf = async () => {
  ...
  try {
    const pdfBlob = await contractsService.downloadContractPdf(contractInstance.value.id);

    // Create download link
    const url = window.URL.createObjectURL(pdfBlob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `contrato-reserva-${reservation.value.id}.pdf`;
    ...
    alert('Error al descargar el contrato. Por favor, inténtelo de nuevo más tarde.');
  } finally {
  ...
  <main class="px-4 sm:px-8 md:px-10 lg:px-16 py-10 w-full min-h-[90dvh] flex flex-col gap-6">
    ...
    <div class="w-full flex flex-col md:flex-row gap-6 text-(--text-color)">
      ...
      <div class="flex flex-col justify-center gap-4 shadow-lg bg-(--background-card-color) rounded-lg p-4 w-full md:w-1/3 max-h-180 overflow-y-auto">
        ...
        <div v-if="contractInstance" class="flex flex-col gap-3 mt-2">
          <h3 class="text-xl font-semibold">Contrato de reserva</h3>
          <button
            @click="downloadContractPdf"
            :disabled="isDownloadingPdf"
            class="flex items-center justify-center gap-2 bg-[var(--secondary-color)] rounded-md py-3 px-4 text-white text-lg hover:cursor-pointer hover:bg-[var(--secondary-color-hover)] transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg v-if="isDownloadingPdf" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            {{ isDownloadingPdf ? 'Descargando...' : 'Descargar contrato PDF' }}
          </button>
          <p class="text-sm text-gray-500">
            Firmado el {{ new Date(contractInstance.signedAt).toLocaleString('es-ES') }}
          </p>
        </div>
